...
do
	mkdir -p $i
	cd $i
	if [ -e $i-qsub.deny ]; then cp $i-qsub.deny $i-qsub.deny-old; fi
	if [ -e $i-user.deny ]; then cp $i-user.deny $i-user.deny-old; fi
	if [ -e $i-mdiag.log ]; then cp $i-mdiag.log $i-mdiag.log-old; fi
	...
	awk '/mdiag.log/ {flag=1;next} /user.deny/ {flag=0} flag {print}' dump > $i-mdiag.log
	awk '/user.deny/ {flag=1;next} /qsub.deny/ {flag=0} flag {print}' dump > $i-user.deny
	awk '/qsub.deny/ {flag=1;next} /^$/ {flag=0} flag {print}' dump > $i-qsub.deny
